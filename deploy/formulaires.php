...
/**
 * Nom du site dans les e-mails d'alerte — SPÉCIFIQUE À CHAQUE SITE (owner,
 * 14/09/2026). Les deux sites partagent ce fichier au octet près ; une alerte
 * signée « Alerte NSY » qui raconte une panne de PRV Concept, et le lecteur ne
 * sait plus lequel des deux appelle au secours. C'est la SEULE ligne qui
 * diffère d'un site à l'autre.
 */
if (!defined('NSY_ALERTE_SITE')) define('NSY_ALERTE_SITE', 'NSY');
...
function nsy_alerte_owner(array $config, string $sujet, string $texte, string $cle = 'generale'): bool
{
    ...
    if (file_exists($temoin) && (time() - (int) filemtime($temoin)) < 86400) {
        nsy_alerte_trace($cle, $sujet, 'ÉTOUFFÉE (témoin de moins de 24 h)');
        return false;
    }
    if (empty($config['smtp_host']) || empty($config['to_address'])) {
        nsy_alerte_trace($cle, $sujet, 'IMPOSSIBLE (smtp_host ou to_address absent de la config)');
        return false;
    }
    @touch($temoin);
    try {
        ...
        $m->CharSet    = 'UTF-8';
        $m->Timeout    = 15;
        $m->setFrom($config['smtp_username'], 'Alerte ' . NSY_ALERTE_SITE);
...
/**
 * Trace d'audit des alertes — une ligne par tentative dans _secret/alertes.log
 * (owner, 14/09/2026 : une alerte attendue qui n'arrive pas ne doit pas laisser
 * à chercher entre « jamais partie », « étouffée par le témoin » et « perdue en
 * route »). Aucune donnée personnelle : date, clé, sujet, issue. Ne lève jamais.
 */
function nsy_alerte_trace(string $cle, string $sujet, string $issue): void
{
    @file_put_contents(
        __DIR__ . '/_secret/alertes.log',
        date('c') . ' | ' . NSY_ALERTE_SITE . ' | ' . $cle . ' | ' . $issue . ' | ' . $sujet . "\n",
        FILE_APPEND
    );
}
