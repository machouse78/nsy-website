...
$modelesEssayes = [];    // ce qui a VRAIMENT été tenté, pour que l'alerte ne mente pas
...
// (owner, 02/09/2026) Question sur les CLIENTS / références / exemples de
// travaux : la règle 4 demande d'inviter à la page Réalisations, mais le modèle
// cite volontiers PRV Concept et Le Cerf Thym sans donner la page qui les
// présente. Ajout déterministe, dans la langue de la réponse, s'il manque.
{
    $dernierMsg = '';
    foreach (array_reverse($body['messages']) as $mm) {
        if (($mm['role'] ?? '') === 'user') { $dernierMsg = mb_strtolower((string) ($mm['content'] ?? '')); break; }
    }
    if ($dernierMsg !== ''
        && preg_match('/\b(clients?|customers?|r[ée]f[ée]rences?|r[ée]alisations?|portfolio|exemples?|examples?|travaux|projets? (?:r[ée]alis|livr)|pour qui|for whom|who do you work)/u', $dernierMsg)
        && !str_contains($reply, 'realisations.html') && !str_contains($reply, 'portfolio.html')) {
        $reply .= replyIsEnglish($reply)
            ? "\n\nSee the delivered sites in detail on the [Portfolio](portfolio.html) page."
            : "\n\nRetrouvez les sites livrés en détail sur la page [Réalisations](realisations.html).";
    }
}
