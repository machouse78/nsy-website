...
Huit [skills Claude Code](https://docs.claude.com/en/docs/claude-code/skills)
...
| **`chatbot-core/`** | **Socle commun** réutilisable du chatbot à mascotte animée (nsy.fr, prv-concept.com, clients) : architecture du widget `.cbot-*`, pipeline de la mascotte (générer → composer → boomerang → dé-bober), **charte graphique par design tokens** (méthode, `reference/charte-graphique.md`), perf + correctif du gel vidéo iOS, étage serveur LLM ancré + garde-fous zéro-invention. Ne contient aucune valeur propre à un site. |
| **`chatbot-nsy/`** | Spécificités NSY du chatbot (**hérite de `chatbot-core`**) : persona « Ansley » (IA affichée, badge Mistral), charte cyan, carte des fichiers (`partials/chatbot.*.html` + `sync-partials`, `.cbot-*` de `style.css`, `chat.php` Mistral+health+RAG), et la spec du FAB (portrait 132×168, halo `cbot-pulse`, greeter). |
...
for s in skill-nsy-website chatbot-core chatbot-nsy frontend-responsive-perf seo-geo-llmo antispam frontend-design video-to-website; do
